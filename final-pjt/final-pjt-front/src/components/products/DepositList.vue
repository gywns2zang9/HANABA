<template>
  ...
  <div>
    <h4 class="product-info">
      ...
      <RouterLink :to="{ name: 'depositDetail', params: { fin_prdt_cd: product.fin_prdt_cd } }" class="detail-link">상세 보기</RouterLink>
    </h4>
  </div>
</template>
...
axios.get(`http://127.0.0.1:8000/api/v1/products/deposit-joins/${productId}/`)  // 1은 적절한 deposit_pk로 대체해야 합니다.
  .then(response => {
    productJoins.value = response.data.num_users
  })
  .catch(error => {
    console.error('Error fetching deposit joins:', error);
  });

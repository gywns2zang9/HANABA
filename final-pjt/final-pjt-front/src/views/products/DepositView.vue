  ...
  <div>
    ...
    <!-- 정기예금 리스트 -->
     <div  class="deposit-list-item">
       <DepositList
       v-for="product in filteredDepositProducts"
...
const depositProducts = store.depositProducts; // 전체 정기예금 리스트

onMounted(() => {
  store.getDeposit(); // 페이지 로딩 시 정기예금 데이터 가져오기
  counterStore.getInfo()
});

// 검색된 정기예금 리스트를 계산된 속성으로 정의
const filteredDepositProducts = computed(() => {
  if (!searchQuery.value) {
    ...
    const searchResult = depositProducts.filter(product => {
...
watch(filteredDepositProducts, (newValue) => {
...
const sortByProductJoins = () => {
  ...
  if (sortDirection.value === 'asc') {
    // 오름차순 정렬
    filteredDepositProducts.value.sort((a, b) => {
